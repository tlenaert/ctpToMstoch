...
names(data)<-c("Reasoning","1","2","3","4","5","6","7")
data[1,1]<-"no **ToM**"
data[2,1]<-"**ToM**"
data <- data[c(2,1),]
...
plot<-ggplot(data=df, aes(x=variable, y=value, fill=Reasoning)) +
  ...
  geom_text(aes(label=value), vjust=1.2, color="white",
            position = position_dodge(0.9), size=9)+
  #scale_fill_brewer(labels=c("**ToM** (0 \u2264 k \u2264 6)","no **ToM** (k = 0)"), palette="Paired", direction=-1)+
  theme_minimal()+
  scale_fill_manual(labels=c("**ToM** (*0 \u2264 k \u2264 6*)","no **ToM** (*k = 0*)"),values=c("#0072B2","#FFCC66"))+
