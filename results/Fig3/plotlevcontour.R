...
#read all the files
flist<-list.files(pattern="ctperrorLallfine.txt")
...
#make smooth contour
mod <- loess(avgl ~ beta + eps, data = data.cont,span=0.2)
df <- with(data.cont, 
...
myplot <-ggplot(df, aes(x=beta, y=eps, z=yvar))+
  ...
  annotate("text", x=0.315, y=0.20, label= "("~beta~"=0.31,"~epsilon~"=0.19)", colour="white", size=7, hjust="left")+
