...
nodes <- read.csv("ctpnodesB31.txt", header=T, as.is=T)
links <- read.csv("ctpedgesB31.txt", header=T, as.is=T)
...
V(net)$label.cex <- 2+ 7.5*V(net)$sd
V(net)$size <- V(net)$sd*25 + 15
...
legend(x=-1.9, y=1.0, c("k=0","k=1", "k=2", "k=3", "k=4"), pch=21,
