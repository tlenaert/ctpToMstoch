...
#make smooth contour
mod <- loess(diff ~ beta + eps, data = data.cont,span=0.05)
df <- with(data.cont, 
           expand.grid(eps = seq(min(eps), max(eps), len = 100),
                       beta = seq(min(beta), 0.2, len = 100)))
...
myplot <-ggplot(df, aes(x=beta, y=eps, z=yvar))+
  ...
  geom_contour(color="white",bins=25)+
  scale_fill_viridis_c(limits = c(-0.15,1.3), breaks = c(-0.25, 0.0, 0.25, 0.75,1.25)) +
